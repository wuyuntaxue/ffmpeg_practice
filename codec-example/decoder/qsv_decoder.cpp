...
namespace Codec {
    ...
    int QSVDecoder::open(AVCodecID codecID, DecodeCallback callback) {
        int                 ret;
        enum AVHWDeviceType device_type = AV_HWDEVICE_TYPE_NONE;

        do {
            const char *device_name = "qsv";
            device_type             = av_hwdevice_find_type_by_name(device_name);
            if (device_type == AV_HWDEVICE_TYPE_QSV) {
                LOG_CHN(INFO, chn_) << "find hwdevice qsv";
            } else {
                LOG_CHN(ERROR, chn_) << "can't find hwdevice qsv";
                while ((device_type = av_hwdevice_iterate_types(device_type)) != AV_HWDEVICE_TYPE_NONE) {
                    LOG_CHN(ERROR, chn_)
                        << "available device types: " << av_hwdevice_get_type_name(device_type);
                }
                return -1;
            }
            ...
            case AV_CODEC_ID_H264:
                pDecodec_ = avcodec_find_decoder_by_name("h264_qsv");
                break;
            case AV_CODEC_ID_HEVC:
                pDecodec_ = avcodec_find_decoder_by_name("hevc_qsv");
                break;
            case AV_CODEC_ID_VP9:
                pDecodec_ = avcodec_find_decoder_by_name("vp9_qsv");
                break;
            case AV_CODEC_ID_VP8:
                pDecodec_ = avcodec_find_decoder_by_name("vp8_qsv");
                break;
            case AV_CODEC_ID_AV1:
                pDecodec_ = avcodec_find_decoder_by_name("av1_qsv");
                break;
            case AV_CODEC_ID_MPEG2VIDEO:
                pDecodec_ = avcodec_find_decoder_by_name("mpeg2_qsv");
                break;
            case AV_CODEC_ID_MJPEG:
                pDecodec_ = avcodec_find_decoder_by_name("mjpeg_qsv");
                break;
            default:
                LOG_CHN(ERROR, chn_) << "codecid: " << codecID << " is not supportted by qsv\n";
                return -1;
            }
            ...
            LOG_CHN(INFO, chn_) << "qsv decoder init done";
            return 0;
    ...
    int QSVDecoder::decode(uint64_t pktid, FGRecord::AVPacketSP fgpkt) {
    ...
    int QSVDecoder::flush(uint64_t ptkid) {
        if (pDecodec_ctx_) {
    ...
    AVBufferRef *QSVDecoder::getHwFramesCtx() {
        if (pDecodec_ctx_) {
